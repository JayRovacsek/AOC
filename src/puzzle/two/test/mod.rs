...
fn test_part_a() {
    ...
    assert_eq!(
        70,
        *interpreter_a.clone().run(0).iter().nth(3).unwrap_or(&0)
    );
    assert_eq!(3500, *interpreter_a.clone().run(0).first().unwrap_or(&0));
    ...
    assert_eq!(&6_i64, interpreter_c.run(0).iter().nth(3).unwrap_or(&0_i64));
    assert_eq!(
        &9_801_i64,
        interpreter_d.run(0).iter().nth(5).unwrap_or(&0_i64)
    );
