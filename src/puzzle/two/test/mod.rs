...
fn test_part_a() {
    ...
    assert_eq!(70, interpreter_a.clone().run_all_outputs(0)[3]);
    assert_eq!(3500, interpreter_a.clone().run_all_outputs(0)[0]);
    ...
    assert_eq!(
        &2_i32,
        interpreter_b.run_all_outputs(0).first().unwrap_or(&0_i32)
    );
    assert_eq!(
        &6_i32,
        interpreter_c
    ...
    assert_eq!(
        &30_i32,
        interpreter_e.run_all_outputs(0).first().unwrap_or(&0_i32)
    );
}
